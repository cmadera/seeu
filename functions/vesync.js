...
const urlLogin = BASE_URL + "/vold/user/login";
const urlDevice = BASE_URL + "/vold/user/devices";
const urlTurn = BASE_URL + "/v1/wifi-switch-1.3/";
...
exports.getDevices = function getDevices(res, pconfig) {
	...
	request.post(get_login(), function (e, r, body) {
		...
		request({url: urlDevice, data: {}, json: true, headers: get_headers(body)}, function (e, r, body) {
            //console.log(body);
			for(var exKey in body) {
				console.log("key: "+exKey+", value: "+body[exKey].cid+", name: "+body[exKey].deviceName);
			}
...
function get_login() {
	return {url: urlLogin, json: {'account':config.username,'devToken':'','password':config.password}};
}

function get_headers(account){
	//console.log({'tk':account.tk,'accountid':account.accountID, 'Content-Type': 'application/x-www-form-urlencoded'});
	return {'tk':account.tk,'accountid':account.accountID, 'Content-Type': 'application/x-www-form-urlencoded'}
}
/*
